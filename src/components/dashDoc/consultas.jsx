...
export default function ProximasConsultasInfo({ info }) {
  ...
  return (
    <div className="flex items-center px-2 xs:px-4 md:px-6 md:py-4 py-2 justify-between border-b border-[#DCDBDB]">
      <div className="flex items-center w-2/3 md:w-[50%] ">
        <span className="mr-2">
          {" "}
          {info.patientPulmonaryHypertensionRisks?.risk ? (
            <RealColorRisk
        ...
        <div className="flex w-full md:w-fit pr-4">
          <Avatars avatar1={info?.patientUser?.avatar} />
        </div>
        <div className="flex gap-2  items-center  w-[100%]">
          <p className="h-10 w-fit flex text-start items-center  text-[rgb(104,104,104)] font-normal text-base leading-6 px-2 py-1">
            {info.patientUser.name} {info.patientUser.lastname}
          </p>
          <div>
            <IconCurrentRouteNav
              className={"w-[1.5rem] hidden md:block mr-10"}
            />
          </div>
        </div>
      ...
      <div className="flex space-x-1 md:space-x-4 ">
        <div onClick={() => handleCokiePatient(info.id, info.patient)}>
