...
export default async function GeneratePDF(user, consultas) {
  ...
  function splitTextIntoParagraphs(text, maxChars) {
    ...
    if (typeof maxChars !== "number" || maxChars <= 0) {
      throw new TypeError(
        'The "maxChars" parameter should be a positive number'
      );
    }
  ...
  const circularAvatarDataURL = await getCircularImageDataURL(
    avatarUrl,
    imageSize
  );
  ...
  doc.text(
    `${user.patientPulmonaryHypertensionGroups?.group || "-"}`,
    120,
    y + 30
  );
  ...
  const antecedentes = [
    {
      ...
      data: user.patientPulmonaryHypertensionRisks?.risk || "-",
    },
    {
      title: "Riesgo quirúrgico:",
      data: user.patientSurgicalRisks?.risk || "-",
    },
    {
      title: "Antecedentes quirúrgicos:",
      data: user.backgrounds?.surgicalBackground || "-",
    },
    {
      title: "Antecedentes patológicos:",
      data: user.backgrounds?.pathologicBackground || "-",
    },
    {
      title: "Antecedentes no patológicos:",
      data: user.backgrounds?.nonPathologicBackground || "-",
    },
    {
    ...
    },
    {
      ...
      data: user.backgrounds?.pharmacologicalBackground || "-",
    },
    ...
    consultas.forEach((consulta) => {
      ...
      const anamnesis = [
        ...
        },
        {
          ...
          data: consulta?.reviewOfSystems || "-",
        },
      ];

      const evolucion = [
        { title: "", data: consulta?.historyOfPresentIllness || "-" },
      ];

      const formattedDrugs = consulta?.drugPrescriptions?.join(", ") || " - ";
      const formattedProcedures =
        consulta?.medicalProcedure?.join(", ") || " - ";
      const dignostic = consulta.diagnostics?.length
        ? consulta.diagnostics?.map((d) => d.diagnosticNotes)
        : "-";
      ...
      const antropometriDetail =
        consulta?.antropetriDetails?.map((sign, index) => ({
          title: antropometricTitle[index] || "",
          data: `${sign?.measure || "-"} ${sign?.measureUnit}`,
        })) || [];

      const vitalSigns =
        consulta?.vitalSigns?.map((sign, index) => ({
          title: vitalSignsTitles[index] || "",
          data: `${sign?.measure || "-"} ${sign?.measureUnit}`,
        })) || [];

      const signosVitales = [...antropometriDetail, ...vitalSigns];
