...
import getPatientRequests from "@/utils/dataFetching/fetching/getPatientRequests";

export default function Requests({id, allDoctors}) {
    ...
    const lastSegmentTextToShow = "Solicitudes"

    const fetchData= async ()=>{
        try {
            
            const response = await getPatientRequests(id)
            if(response){
                setRequest(response)
                setisLoading(false)
    ...
    const sortedRequests = isSorted
        ? [...filteredRequest].sort((a, b) => a.name.localeCompare(b.name))
        : filteredRequest;
                ...
                <div className=" justify-center w-[100%] items-center h-[100%] bg-[#FAFAFC] ">
                    {isLoading ? (
                        ...
                        filteredRequest.map((request) => (
                            ...
                            <div key={request.id} className="w-[100%] h-fit flex border-b items-center  border-b-[#cecece]  bg-white">
                                ...
                                <div className="grid text-center grid-cols-4 w-[100%] pr-3  justify-center md:w-[100%] md:text-left md:grid-cols-6 items-center  py-2 bg-white z-10">
                                    ...
                                    <div className="text-[#5F5F5F] ">
                                        {Fecha(request.createdAt, 4)}
                                    </div>
                                    <div className="text-[#5F5F5F] md:block hidden ">
                                        {Hora(request.createdAt)}
                                    </div>
                                    <div className="text-[#5F5F5F] ">
                                        {request?.physicianReq?.name}       {request?.physicianReq?.lastname}
                                    </div>
                                    <div className="text-[#5F5F5F]">
                                        {request?.reqTypes}
                                    </div>
                                                ...
                                                items: [
                                                    {
                                                        ...
                                                        onClick: () => { setSelectedRequest(request); setShowModalModify(true) },
                                                        icon: <IconEditar color={"#B2B2B2"} />,
                                                    },
                                                    {
                                                        label: "Eliminar solicitud",
                                                        onClick: () => { setSelectedRequest(request); setShowModalDelete(true) },
                                                        icon:
                                                            <IconClose2 />
                                                        ,
                                                    },
