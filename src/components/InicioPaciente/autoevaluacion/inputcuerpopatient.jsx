...
export default function CuerpoPatient() {
  ...
  const onSubmit = async (data) => {
    ...
    const dataSendFinal = {
      ...mappedData,
      painAreas: mappedCuerpo,
      painOwner: Number(userId),
    };

    console.log(dataSendFinal);
    try {
      const response = await ApiSegimed.post(
        `/self-evaluation-event/pain-map`,
        dataSendFinal,
        {
          headers: { token: token },
        }
    ...
    <div className="flex flex-col w-full h-full ">
      <form onSubmit={handleSubmit(onSubmit)}>
            ...
            {selectedMuscles.length > 0 &&
              selectedMuscles[0].muscles.map((muscle, index) => (
                <div
                  ...
                  <label htmlFor={`muscle-note-${index}`}>
                    <IconConsulta />
        ...
        <div className="w-full flex justify-center p-5">
          <button
            type="submit"
            className="bg-greenPrimary py-2 px-4 items-center justify-center  flex rounded-lg gap-2 w-fit">
            <p className="block text-white font-bold">Enviar autoevaluacion</p>
          </button>
        </div>
      </form>
    </div>
