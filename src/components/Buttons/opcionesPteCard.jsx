    ...
    return (
        <details className="relative">
            ...
            <ul className="absolute bg-white z-50 p-2 text-start text-[#686868] font-normal text-base leading-6 w-64 right-0 border-2 border-[#D7D7D7] rounded-lg gap-4 mt-2 shadow-lg">
                <li className='flex items-center gap-2 font-medium text-sm leading-4'><IconAccion />Acciones</li>
                <li className='font-normal text-base leading-8 ml-6 flex items-center gap-2'><IconMiniCalendar />Agendar Consulta</li>
                <li className='font-normal text-base leading-8 ml-6 flex items-center gap-2'><IconTratamiento />Añadir tratamiento</li>
                <li className='font-normal text-base leading-8 ml-6 flex items-center gap-2'><IconTStar3 />Agregar a favoritos</li>
                <li className='flex items-center gap-2 font-medium text-sm leading-4'><IconInfo />Información</li>
                <li className='font-normal text-base leading-8 ml-6 flex items-center gap-2'>
                    <Link className='flex items-center gap-2' href={`${rutas.Doctor}${rutas.Pacientes}${rutas.Historia_Clinica}/${paciente.id}`}>
                        <IconClinicalHistory />Ver Historia Clínica
                    </Link>
                </li>
                <li className='font-normal text-base leading-8 ml-6 flex items-center gap-2'>
                    <Link className='flex items-center gap-2' href={`${rutas.Doctor}${rutas.Pacientes}/${paciente.id}`}>
                        <IconPersonalData />Ver datos Personales
                    </Link>
                </li>
                <li className='font-normal text-base leading-8 ml-6 flex items-center gap-2'>
                    <Link className='flex items-center gap-2' href={`${rutas.Doctor}${rutas.Pacientes}${rutas.Historial}/${paciente.id}`}>
                ...
                <li className='flex items-center gap-2 px-3 cursor-pointer'>
                    <IconMessages /> <button onClick={()=>{socket.emit("createChat",{id})}}>Iniciar Chat</button>
                </li>
                </Link>
                <li className='font-normal text-base leading-8 ml-6 flex items-center gap-2'><IconGeolocation />Ver Geolocalización</li>
            </ul>
        </details>
    )
}