...
export default function ClinicalHistory({ edit, paciente = {}, title, patologias = [], dataType, saveData }) {
    ...
    useEffect(() => {
        if (paciente[dataType] && typeof paciente[dataType] === 'object') {
            ...
            setPatologiasRestantes(patologias.filter(patologia => !existingPatologias.includes(patologia)));
        }
    ...
    const handleSelectChange = (e) => {
        ...
        if (selectPatologia !== 'Añadir') {
            ...
            const nuevasPatologiasRestantes = patologiasRestantes.filter(patologia => patologia !== selectPatologia);
    ...
    const handleDelete = (key) => {
        if (paciente[dataType] && paciente[dataType][key]) {
            ...
            setNuevasPatologias(nuevasPatologias.filter(patologia => patologia !== key));
        }
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <details>
                    ...
                    <ul className="bg-white py-5 px-2 w-full right-4 gap-3 z-10">
                        {paciente[dataType] && typeof paciente[dataType] === 'object' ? Object.entries(paciente[dataType]).map(([key, value]) => (
                            <li key={key} className="flex justify-start items-center p-4">
                                ...
                                {edit ? (
                                    ...
                                    <span className='w-1/2 text-start px-6 py-1 '>{value}</span>
                                )}
                            </li>
                        )) : null}
                        {nuevasPatologias.map((patologia, index) => (
                            <li key={index} className="flex justify-start items-center px-3 pr-10 mt-2">
                            <label className='flex items-center w-1/2 font-medium ml-2 '>
                                <IconCircle  /> <span className='ml-4'>{patologia}</span>
                            </label>
                                {edit ? (
                                    <div className='w-1/2 flex justify-between items-center gap-2'>
                                        <input
                                        ...
                                        />
                                        <button type="button" onClick={() => handleDelete(patologia)}>
                                            <IconDelete />
                                        </button>
                                    </div>
                                ) : (
                                    <span className='w-1/2 text-start px-6 py-1 '>{watch(patologia) || ""}</span>
                                )}
                            </li>
                        ))}
                    </ul>
