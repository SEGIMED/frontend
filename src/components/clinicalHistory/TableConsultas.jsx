...
const PriorityIcon = ({ priority }) => {
    switch (priority.toLowerCase()) {
        case "alta":
            return <IconAlarmRed />;
        case "media": IconClinicalHistory
        ...
        <div className="h-full flex flex-col">
            <div className="">
                {consultas?.map((paciente, index) => (
                    <div key={index} className="grid grid-cols-7 items-center border-b border-b-[#cecece] pr-6 py-2 bg-white w-full h-fit ">
                        <div className="flex justify-center"><IconConsulta /></div>
                        ...
                        <div className="text-[#5F5F5F] justify-center"><AlarmButtonDoc id={paciente.id} handleStatus={handleStatus} /></div>
                    </div>
                ))}
            </div>
        </div>
