...
const TurnosButton = ({idDoc}) => {
    ...
    useEffect(() => {
        if (pathname === `${rutas.PacienteDash2}${rutas.Mis_turnos}${rutas.Pasadas}`) {
            setFlag(true);
        } else {
            setFlag(false);
        }
    ...
    return (
        ...
        <details className="relative w-[60%] md:w-full">
            ...
            {!flag ? (
                <ul className="flex flex-col absolute p-2 bg-white z-40 w-80 right-0 border-2 border-[#D7D7D7] rounded-lg shadow-lg mt-1">
                    ...
                    <div className="flex flex-col gap-4 p-5">
                        ...
                        <li onClick={cancelTurn} className="flex items-center gap-3 cursor-pointer">
                            <IconCancel />
                            Cancelar Turno
                        </li>
                    </div>
         ...
         {isReviewModalOpen && (
            <ReviewModal onClose={() => setIsReviewModalOpen(false)} idDoc={idDoc} />
          )}
          </div>
    );
};
