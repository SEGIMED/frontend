...
export default function HomePte() {
    ...
    const filteredDocs = listaDocs?.filter(
        (doc) =>
    ...
    const docWithConsultas = filteredDocs.map(doc => {
        const consulta = scheduledConsultas.find(consulta => consulta.physician === doc.id && consulta.typeOfMedicalConsultation === 1);
        return { ...doc, consulta };
    });
    ...
    return (
        <div className="h-full text-[#686868] w-full flex flex-col">
            <div className="flex justify-between border-b border-b-[#cecece] px-6 py-2">
                ...
                <button
                    className="flex px-6 py-2 rounded-xl gap-1 items-center bg-[#487FFA]"
                    onClick={() => { router.push(`${rutas.PacienteDash}${rutas.Preconsulta}${rutas.Teleconsulta}`) }}>
            ...
            <div className="flex flex-col items-start justify-center w-full">
                {sortedDocs?.map(doc => (
                    <DoctorCardConsulta key={doc.id} doctor={doc} consulta={doc.consulta} button={<Ver id={doc.id}
