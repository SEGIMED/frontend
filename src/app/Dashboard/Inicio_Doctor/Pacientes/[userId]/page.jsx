    ...
    const getPatient = async () => {
        try {
            ...
            const [response1, response2] = await Promise.all([
                ...
                ApiSegimed.get(`/patient-details?id=${id}`, { headers: { "token": token } })
            ]);
        ...
        <div className="h-full flex flex-col">
            <div className="flex justify-between items-center gap-2 px-4 py-3 border-b border-b-[#cecece]">
                <div className='flex items-center '>
                    ...
                    <p className="text-lg font-normal leading-6 text-[#5F5F5F] ">Detalle del paciente</p>
                </div>
                <div>
            ...
            <div>
                <Detail title={'Geolocalizacion:'} data={<Elboton nombre={"Ubicacion"} onPress={openModal} />} />
                <Detail title={'Nombre:'} data={user?.name} />
                <Detail title={'Apellido:'} data={user?.lastname} />
                <Detail title={'Edad:'} data={CalcularEdad(user?.sociodemographicDetails?.birthDate)} />
                <Detail title={'Sexo:'} data={user?.sociodemographicDetails?.genre} />
                <Detail title={'Documento:'} data={user?.idNumber} />
                <Detail title={'Nacionalidad:'} data={user?.nationality === 1 ? 'Colombiano' : user.nationality === 2 ? 'Argentino' : user.nationality} />
                <Detail title={'Fecha de nacimiento:'} data={user?.sociodemographicDetails?.birthDate} />
                <Detail title={'Telefono de contacto:'} data={<Elboton nombre ={"Solicitar permiso"} />} />
                <Detail title={'Telefono de urgencia:'} data={<Elboton nombre ={"Solicitar permiso"} />} />
                <Detail title={'Correo electronico:'} data={<Elboton nombre ={"Solicitar permiso"} />} />
                <Detail title={'Lugar de domicilio:'} data={<Elboton nombre ={"Solicitar permiso"} />} />
                <Detail title={'Fecha del Diagnostico Principal:'} data={user?.lastname} />
                <Detail title={'Lugar de atencion medica:'} data={user?.lastname} />
                <Detail title={'Medico a cargo:'} data={user?.lastname} />
                <Detail title={'Estado:'} data={user?.sociodemographicDetails?.civilStatus} />
                <Detail title={'Ultima conexion:'} data={LastLogin(user?.lastLogin)} />
              
            </div>
            {showModal && <MapModalPte onClose={() => setShowModal(false)} patient={user} />}
        </div>
    );
};
