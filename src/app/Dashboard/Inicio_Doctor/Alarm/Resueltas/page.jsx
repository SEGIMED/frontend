...
export default function HomeDoc() {
    ...
    const getAlarms = async (headers) => {
        try {
            ...
            if (response.data) {
                const inactiveAlarms = response.data.filter(alarma => alarma.solved === true);
                ...
                const mappedAlarms = inactiveAlarms.map(alarm => ({
                    ...alarm,
                    highestPriority: getHighestPriority(alarm.questionsPriority.map(p => p.split(': ')[1]))
                }));
    ...
    return (
        <div className="h-full flex flex-col">
            <div className="flex items-center justify-between border-b border-b-[#cecece] pl-5 pr-6 py-2 bg-white sticky top-0 z-50">
                {/* <Ordenar /> */}
                <div></div>
            ...
            <div className="flex items-center justify-between border-b border-b-[#cecece] pl-2 pr-6 py-2 bg-white sticky top-14 z-10">
                ...
                <p className="font-bold text-[#5F5F5F] w-1/6">HTP</p>
                <p className="font-bold text-[#5F5F5F] w-1/6">Motivo de alarma</p>
                <p className="font-bold text-[#5F5F5F] w-1/6">Tiempo de respuesta</p>
            </div>
