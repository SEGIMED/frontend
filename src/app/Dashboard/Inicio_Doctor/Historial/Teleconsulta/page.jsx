...
export default function HomeDoc() {
    ...
    const filteredPatients = listaPacientes?.filter(paciente =>
    ...
    const patientsWithConsultas = filteredPatients.map(paciente => {
        const consulta = scheduledConsultas.find(consulta => consulta.patient === paciente.id && consulta.typeOfMedicalConsultation === 2);
    ...
    return (
        <div className="h-full text-[#686868] w-full flex flex-col">
            <div className="flex justify-between border-b border-b-[#cecece] px-6 py-2">
                ...
                <button 
                    className="flex px-6 py-2 rounded-xl gap-1 items-center border-solid border-[#487FFA] border-2 bg-white" 
                    >
                    <IconFolder/>
                    <p className="text-start text-[#487FFA] font-bold text-base leading-5">Pasadas</p>
            ...
            <div className="flex flex-col items-start justify-center w-full">
                {sortedPatients?.map(paciente => (
                    <PatientCardConsulta key={paciente.id} paciente={paciente} consulta={paciente.consulta} button={<OptPteHistorial id={paciente.id}
